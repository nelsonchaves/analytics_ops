...
RSpec.describe AnalyticsOps::Configuration::Writer do
  it "strictly validates result booleans" do
    expect { described_class::Result.new(path: "config/analytics_ops.yml", created: "yes") }
      .to raise_error(ArgumentError, /distinct booleans/)
  end
  ...
  it "safely appends a missing profile without removing existing settings or comments" do
    Dir.mktmpdir("analytics-ops-writer") do |directory|
      path = File.join(directory, "analytics_ops.yml")
      ...
      result = described_class.new.write_minimal(path, profile: "client_b", property_id: "987654321")
      configuration = AnalyticsOps::Configuration.load(path)

      expect(result).to be_updated
      expect(result).not_to be_created
      expect(configuration.profile("production").key_events).to eq(["purchase"])
      expect(configuration.profile("client_b").property_id).to eq("987654321")
      expect(File.read(path)).to include("# Keep this operator note.")
    end
  end

  it "fills the untouched Rails generator placeholder during setup" do
    Dir.mktmpdir("analytics-ops-writer") do |directory|
      path = File.join(directory, "analytics_ops.yml")
      File.write(path, <<~YAML)
        # Generated by Analytics Ops.
        version: 1
        profiles:
          production:
      ...
      expect(result).to be_updated
      expect(File.read(path)).not_to include("GA4_PROPERTY_ID")
